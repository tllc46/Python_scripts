...
fig.plot(x=df.loc[df["status"].isin("O"),"longitude"],y=df.loc[df["status"].isin("O"),"latitude"],size=2*abs(df.loc[df["status"].isin("O"),"residual"])+0.1,cmap=True,fill=df.loc[df["status"].isin("O"),"residual"],style="cc",pen=True)
fig.plot(x=df.loc[df["status"].isin("X"),"longitude"],y=df.loc[df["status"].isin("X"),"latitude"],style="+0.15c")
